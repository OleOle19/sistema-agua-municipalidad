...
async function loadRowsFromExcel(filePath, requestedSheetName = "") {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(filePath);
  const fallbackSheet = workbook.worksheets[0];
  const sheet = requestedSheetName
    ? (workbook.getWorksheet(requestedSheetName) || workbook.getWorksheet(String(requestedSheetName).toUpperCase()) || fallbackSheet)
    : fallbackSheet;
  if (!sheet) throw new Error(`No se encontro hoja valida en ${path.basename(filePath)}.`);
...
async function applyChange(client, item, importUser) {
  ...
  if (status === "replace") {
    ...
    `, [idsPago, fechaObjetivo, importUser]);
    await refreshReciboEstado(client, match.id_recibo);
  ...
  if (status === "insert") {
    ...
    `, [Number(match.id_recibo), row.abono, row.fecha, importUser]);
    await refreshReciboEstado(client, match.id_recibo);
...
async function main() {
  ...
  const requestedSheetName = args[1] || "";
  if (!inputArg) {
    throw new Error("Uso: node scripts/sincronizar_pagos_excel_2026.js <ruta-xlsx> [hoja] [--apply]");
  }

  const excelPath = path.resolve(process.cwd(), inputArg);
  const { sheetName, rows } = await loadRowsFromExcel(excelPath, requestedSheetName);
  if (rows.length === 0) {
    throw new Error(`No se encontraron pagos validos en hoja ${sheetName}.`);
  }
  const importUser = `IMPORTACION_${normalizeName(sheetName).replace(/\s+/g, "_") || "EXCEL"}_2026`;
  ...
  try {
    ...
    console.log(JSON.stringify({
      archivo: excelPath,
      hoja: sheetName,
      usuario_cajero_objetivo: importUser,
    ...
    for (const item of [...replaceItems, ...insertItems, ...adjustItems]) {
      const result = await applyChange(client, item, importUser);
...
main().catch((err) => {
  console.error("Error sincronizando pagos desde Excel 2026:", err.message);
  process.exitCode = 1;
});
