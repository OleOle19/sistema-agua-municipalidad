...
const run = async () => {
  ...
  try {
    ...
    if (files.length === 0) {
      console.log("No se encontraron migraciones en server/sql/migrations.");
      return;
    }

    const appliedRows = await client.query("SELECT file_name, checksum_sha256, applied_at FROM schema_migrations");
    const appliedMap = new Map(
      (appliedRows.rows || []).map((r) => [String(r.file_name), String(r.checksum_sha256 || "")])
    );
    ...
    for (const file of files) {
      ...
      if (appliedChecksum !== checksum) {
        throw new Error(
          `La migracion ${file} ya fue aplicada con otro checksum. No la modifiques; crea una nueva migracion.`
        );
      }
      console.log(`[OK] ${file}`);
    }
...
run()
  .catch((error) => {
    console.error(`Error de migraciones: ${error.message}`);
    process.exit(1);
  });
